...
DATA_URL = (
    "https://raw.githubusercontent.com/climatepolicyradar/open-data/main/data/USA/"
)
...
def search_relevant_urls(keywords: List[str], n: int = 3) -> Dict[str, str]:
    ...
    for k in keywords:
        dump_all_idx.extend(
            [
                ...
                if val_in_list(
                    k.strip().lower(), list(map(str.lower, d["document_keyword"]))
                )
            ]
        )
    ...
    return {
        ...
        for title, url in [[d["document_name"], d["document_url"]] for d in data]
    }
